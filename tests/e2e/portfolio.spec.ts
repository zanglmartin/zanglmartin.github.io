...
for (const route of routes) {
  test(`${route.path} loads directly with accessible content`, async ({
    page,
  }) => {
    const errors: string[] = [];
...
test("desktop navigation reaches every primary section", async ({
  page,
}, testInfo) => {
  test.skip(testInfo.project.name !== "desktop");
  await page.goto("./");

  for (const { label: item, to } of portfolio.navigation) {
    const link = page
...
test("homepage presents four company stories in the selected order", async ({
  page,
}) => {
  await page.goto("./");
  const cards = page.locator(".case-card");
  await expect(cards).toHaveCount(4);
  for (const [index, id] of portfolio.featuredWork.entries()) {
    const work = portfolio.work.find((work) => work.id === id)!;
    await expect(cards.nth(index)).toContainText(work.client ?? work.company);
  }
});
...
test("theme follows the system and persists one local preference", async ({
  page,
}, testInfo) => {
  test.skip(testInfo.project.name !== "desktop");
...
test("contact and GitHub links use the expected safe destinations", async ({
  page,
}) => {
  await page.goto("./");
  const links = [
    {
      name: /^(Connect|Message on LinkedIn)(?: |$)/,
      href: "https://www.linkedin.com/in/martinzangl/",
    },
    { name: "Primary GitHub", href: "https://github.com/mizangl" },
    {
      name: "Portfolio repository",
      href: "https://github.com/zanglmartin/zanglmartin.github.io",
    },
  ];
...
test("keyboard focus, skip navigation, and reduced motion remain accessible", async ({
  page,
}, testInfo) => {
  test.skip(testInfo.project.name !== "desktop");
...
for (const theme of ["light", "dark"] as const) {
  test(`story sections and legacy links work in ${theme} mode`, async ({
    page,
  }) => {
    await page.emulateMedia({ colorScheme: theme });
    await page.goto("./case-studies/mobile-reliability/");
    await expect(page.locator("html")).toHaveAttribute("data-theme", theme);
    await page
      .getByRole("navigation", { name: "Story sections" })
      .getByRole("link", { name: "Outcomes", exact: false })
      .click();
    ...
    await page.goto("./case-studies/fintech-architecture/");
    await page.getByRole("link", { name: "PayPal ↗", exact: true }).click();
    await expect(page).toHaveURL(/case-studies\/paypal$/);
  });
}
