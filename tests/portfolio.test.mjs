...
const routes = routeManifest.map((route) => ({
  ...
  path: canonicalPath(route.path).slice(1),
...
test("pre-renders exactly the expected routes at the GitHub Pages root", async () => {
  ...
  const relativeFiles = htmlFiles
    .map((file) => path.relative(buildRoot.pathname, file))
    .sort();
  ...
  for (const route of routes) {
    ...
    assert.ok(
      ...
      `${route.output} should contain internal references`,
    );
    for (const reference of internalReferences) {
      assert.ok(
        !reference.startsWith("/zanglmartin"),
        `${route.output} contains the old Pages base path: ${reference}`,
      );
    }
  }

  const JavaScriptFiles = await collectFiles(
    new URL("assets/", buildRoot).pathname,
    ".js",
  );
  const JavaScript = (
...
test("publishes unique complete metadata for every route", async () => {
  ...
  for (const route of routes) {
    ...
    assert.equal(
      ...
      route.path.startsWith("case-studies/") &&
        route.path !== "case-studies/" &&
        route.path !== "case-studies/fintech-architecture/"
        ? "article"
        : "website",
    );
  }
...
test("sitemap and robots enumerate the production site", async () => {
  ...
  assert.deepEqual(
    urls,
    routes.map((route) => `${siteUrl}${route.path}`),
  );
...
test("does not publish private CV contact or identity details", async () => {
  ...
  const readableFiles = [...sourceFiles, ...publicFiles, ...buildFiles].filter(
    (file) => /\.(?:ts|tsx|css|txt|xml|html|js|md|json)$/.test(file),
  );
  const publicText = (
    await Promise.all(readableFiles.map((file) => readFile(file, "utf8")))
  )
    .join("\n")
    .toLowerCase();
  const pdfBuffer = await readFile(
    new URL("public/cv/martin-zangl-cv-2026.pdf", root),
  );
  ...
  for (const value of sensitive) {
    assert.equal(
      ...
      false,
      `published files contain ${value}`,
    );
    assert.equal(
      pdfText.includes(value),
      false,
      `sanitized PDF contains ${value}`,
    );
  }
